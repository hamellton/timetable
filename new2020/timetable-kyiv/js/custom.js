  window.addEventListener('load', function() {
      ...
      function ClickHandler() {
          ...
          return function() {
              ...
              $('#campaign, #typeevent,  #theme, #additionaltheme, #additionalbook, #school, #time, #place, #managers, #eventoutline,#eventlink, #comment, #driver, #type, #book, #numberc, #datesend, #advertcomment, #competitioncomment, #certificatecomment, #invitationcomment, #linkcomment, #reportcomment, #offercomment').val('');
              $('#sectorpromoted :input, #certificate :input, #thankyouletter :input').attr('checked', false);
              $('#eventid').val(0);
      ...
      addevent.onsubmit = function() {
          ...
          var testEvent = document.getElementsByName('test');
          var statusb = $("input[name='statusb']:checked").val();
          ...
          for (var i = 0, length = testEvent.length; i < length; i++) {
              if (testEvent[i].checked) {
                  testEvent = testEvent[i].value;
                  break;
              }
          }
          ...
          forJSON.campaign = this.elements.campaign.value;
          forJSON.statusEvent = statusEvent;
          forJSON.testEvent = testEvent;
          forJSON.statusb = statusb;
          ...
          forJSON.eventoutline = this.elements.eventoutline.value;
          forJSON.eventlink = this.elements.eventlink.value;
          ...
          forJSON.sectorpromoted = getCheckbox('#sectorpromoted input:checked');
      ...
      function addEventStart() {
          ...
          xhr.onreadystatechange = function() {
              ...
              DataFromJSON.forEach(function(event, i, DataFromJSON) {
                  ...
                  wrapper.setAttribute('data-sectorpromoted', event.sectorpromoted);
                  wrapper.setAttribute('data-thank', event.thankYouLetter);
                  eventStatus = event.statusEvent;
                  eventTest = event.testEvent;
                  console.log(`${event.testEvent} server`);
                  eventstatusb = event.statusb;
                  date = event.date;
                  methodist = event.methodist;
                  typeEvent.innerHTML = event.typeEvent;
                  typeEvent.classList.add('typeevent');
                  campaign.innerHTML = event.campaign;
                  ...
                  eventoutline.innerHTML = event.eventoutline;
                  eventoutline.classList.add('eventoutline');
                  eventlink.innerHTML = event.eventlink;
                  ...
                  // sectorpromoted.innerHTML = event.sectorpromoted;
                  ...
                  // sectorpromoted.classList.add('sectorpromoted');
                  ...
                  buttonUpdate.addEventListener('click', function() {
                      ...
                      var sectorpromoted = $(this).parent().attr('data-sectorpromoted');
                      sectorpromoted = sectorpromoted.split(',');
                      var thank = $(this).parent().attr('data-thank');
                      thank = thank.split(',');

                      document.querySelector("#typeevent").value = $(allInfoEvent).filter(".typeevent").html();
                      document.querySelector("#campaign").value = $(allInfoEvent).filter(".campaign").html();
                      ...
                      document.querySelector("#eventoutline").value = $(allInfoEvent).filter(".eventoutline").html();
                      document.querySelector("#eventlink").value = $(allInfoEvent).filter(".eventlink").html();
                      ...
                      if (parent.classList[0] === 'testing-1') {
                          document.getElementById('test-1').checked = true;
                      } else if (parent.classList[0] === 'testing-2') {
                          document.getElementById('test-2').checked = true;
                      } else if (parent.classList[0] === 'testing-3') {
                          document.getElementById('test-3').checked = true;
                      }
                      ...
                      $(sectorpromoted).each(function(i, element) {
                          $('input[value="' + element + '"]').prop('checked', true);
                      });
                  ...
                  wrapper.appendChild(eventoutline);
                  wrapper.appendChild(eventlink);
                  ...
                  // wrapper.appendChild(sectorpromoted);
